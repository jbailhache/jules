...
function htmlTableToCals(string $html): string
{
    ...
    // 8) extraire les contenus situés avant et après la table HTML
    $prefix = "";
    $suffix = "";
    ...
    if ($body) {
        ...
        foreach ($body->childNodes as $node) {
            if ($node->isSameNode($table)) {
                $beforeTable = false;
                continue;
            }
            if ($beforeTable) {
                $prefix .= $serializeNode($node);
            } else {
                $suffix .= $serializeNode($node);
            }
        }
    }

    // 9) génération finale en utilisant spanMatrix (compteurs de rowspan)
    $out = [];
